...
def load_plugins():
    for fn in glob.glob('plugins' + os.sep + '*.json'):
        f = open(fn)
        data = json.load(f)
        if data["name"] in all_plugins: # we should throw if this is true, we do NOT want several plugins w/same name
            raise 'Plugin names must be unique - "'+data["name"]+'" is already used'
        else :
